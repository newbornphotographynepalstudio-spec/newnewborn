...
| `settings` (single `site` document) | **Implemented** (Phase 11), holds `socialLinks` and `seo` (Global SEO — `lib/settings/data.ts`). Admin-editable via `/admin/settings/`, which also surfaces live, read-only system status (Firestore/Supabase Storage/Resend/Google Places configuration) computed on each load, never stored. Deliberately the *only* settings document — General/Contact/Business Info were evaluated in Phase 12 and kept code-controlled (`lib/data/contact.ts`): stable business facts that change rarely enough that a code review on change is a feature, not friction, and splitting them into more Firestore documents would just be more places for the one real source of truth to drift. `saveSocialLinks` revalidated only `routes.home` until Phase 15, live-verified as a real bug: Footer() (social links' only consumer) lives in `app/(site)/layout.tsx`, wrapping every public route, so every page but the homepage kept showing stale social links after a save — fixed to `revalidatePath("/", "layout")`, the same layout-wide form `saveGlobalSeo` already used correctly. |
...
- Newborn session packages are real, published pricing (Phase 4,
  ...
  testimonials) — see `lib/data/reviews.ts`. The homepage *is able* to
  fetch real reviews from the Google Places API (New) at request time
  (`lib/reviews/google-places.ts`) if `GOOGLE_PLACES_API_KEY`/
  `GOOGLE_PLACES_PLACE_ID` are ever set, but that stays **intentionally
  disabled** — this is a low-budget project and connecting it would
  require attaching a Google Cloud Billing account, which is out of
  scope. Do not set either variable; see docs/SETUP.md, "Google Reviews
  — intentionally disabled" for the reasoning. The empty state is the
  correct, permanent behavior here, not a placeholder waiting to be
  finished.
